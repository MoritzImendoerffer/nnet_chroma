...
Because the return dim of the INN is also 2, I did modifiy the loss function so that both outputs should be the same.
"""
...
class PhysicsInformedNN():
    ...
    def mse_loss(self):
        ...
        loss = torch.mean((self.u - u_pred) ** 2) + torch.mean(f_pred ** 2) + torch.mean((u1_pred - self.t) ** 2)
        return loss
